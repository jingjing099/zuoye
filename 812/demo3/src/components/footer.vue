<template>
  <div class="footer">
    <div>
      <div class="top">
        <span>
          <i class="el-icon-success">7天无理由退换货</i>
        </span>
        <span>
          <i class="el-icon-success">满99元全场免邮</i>
        </span>
        <span>
          <i class="el-icon-success">100%品质保证</i>
        </span>
      </div>
      <div class="bot">
        <!-- 图片 -->
        <p>
          <a href="https://github.com/hai-27/vue-store">
            <img
              src="data:image/png;base64,"
              alt=""
            />
          </a>
        </p>
        <div class="list">
          <span>首页</span>
          <span class="gang">|</span>
          <span>全部商品</span>
          <span class="gang">|</span>
          <span>关于我们</span>
        </div>
        <p class="di">商城版权所有©2012-2021</p>
      </div>
    </div>
  </div>
</template>
<style lang="scss" scoped>
.top {
  display: flex;
  justify-content: space-around;
  // align-items: center;
  width: 100%;
  line-height: 150px;
  font-size: 25px;
  color: #fff;
}
.footer {
  width: 1225px;
}
img {
  width: 80px;
  height: 80px;
}
.list{
  margin-top: 10px;
  width: 100%;
  text-align: center;
  color: #848484;
  font-size: 20px;
  .gang{
    margin: 0 20px;
  }
}
.di{
  color: #848484;
  margin-top: 15px;
  text-align: center;
}
</style>